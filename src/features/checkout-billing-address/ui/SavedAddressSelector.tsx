...
import { useSavedAddressSelector } from "../lib/useSavedAddressSelector";
import type { SavedAddressSelectorProps } from "../model/interface";
...
export default function SavedAddressSelector({
  ...
  const {
    ...
    handleEditSuccess,
  } = useSavedAddressSelector(addresses, onSelectAddress);
          ...
          <Card className="p-4">
            <div className="flex items-start space-x-3">
              ...
              <Label htmlFor="address-new" className="flex-1 cursor-pointer">
                <div className="font-medium">{t("add_new_address")}</div>
              </Label>
            </div>
          </Card>
        </RadioGroup>
